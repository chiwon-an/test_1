...
const FilePreviewCard: React.FC<FilePreviewCardProps> = ({ file, onRemove }) => {
  ...
  return (
    <div className="relative group flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border border-border bg-muted animate-fade-in transition-all hover:border-muted-foreground">
      {isImage ? (
...
const PastedContentCard: React.FC<PastedContentCardProps> = ({ content, onRemove }) => {
  return (
    <div className="relative group flex-shrink-0 w-28 h-28 rounded-2xl overflow-hidden border border-border bg-card animate-fade-in p-3 flex flex-col justify-between shadow-sm">
...
const ModelSelector: React.FC<ModelSelectorProps> = ({ models, selectedModel, onSelect }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute bottom-full right-0 mb-2 w-[260px] bg-card border border-border rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col p-1.5 animate-fade-in origin-bottom-right">
          {models.map((model) => (
            <button
              key={model.id}
              onClick={() => {
          ...
          <button className="w-full text-left px-3 py-2.5 rounded-xl flex items-center justify-between group transition-colors hover:bg-muted text-foreground">
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-4">
      ...
      {suggestedPrompts.length > 0 && !message && (
        <div className="flex flex-nowrap justify-start gap-2 px-2 md:px-0 overflow-hidden">
          {suggestedPrompts.slice(0, 3).map((prompt, i) => (
            <button
              key={i}
              onClick={() => handleSuggestedPromptClick(prompt)}
              className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-4 py-2 text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground whitespace-nowrap flex-shrink-0"
            >
              {prompt}
            </button>
          ))}
      ...
      >
        ...
        <div
          className="
          flex flex-col mx-2 md:mx-0 items-stretch transition-all duration-200 relative z-10 rounded-2xl cursor-text border border-border
          shadow-[0_0_15px_rgba(0,0,0,0.08)] hover:shadow-[0_0_20px_rgba(0,0,0,0.12)]
          focus-within:shadow-[0_0_25px_rgba(0,0,0,0.15)]
          bg-card font-sans antialiased
        "
        >
        ...
        {/* Drag Overlay */}
        {isDragging && (
          <div className="absolute inset-0 bg-muted/90 border-2 border-dashed border-primary rounded-2xl z-50 flex flex-col items-center justify-center backdrop-blur-sm pointer-events-none">
        ...
        <div className="text-center mt-4">
          <p className="text-xs text-muted-foreground">AI가 요리 레시피를 추천해드립니다. 궁금한 것을 물어보세요!</p>
        </div>
      </div>
    </div>
  )
